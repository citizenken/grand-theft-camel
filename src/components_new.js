	...
	Crafty.c('LeadCamel', {
		...
		init: function() {
			this.requires('Actor, Fourway, Collision, SpriteAnimation, spr_lead_camel_white')
				.fourway(3)
				.attr({followers:this.followers, steps:0, direction:null, path:{}})
				.bind('Moved', function(oldLocation) {
					this.steps++
					// console.log(oldLocation)
					var index = this.followerNumber
					if ((this.steps % (Game.map_grid.tile.width/2) === 0) && this.followers.indexOf(index) == -1) {
						this.path[index] = {x:this.x, y:this.y};
						this.followerNumber++
					}
